...
var newHtml="";if ($.trim(im[ram_G[indexGame]])){newHtml='<img id="imaL" class="imaLeft" alt="Ima" src="denaro3_resources/media/'+im[ram_G[indexGame]]+'">';}
newHtml=newHtml+te[ram_G[indexGame]];$("#actText").html(newHtml);
if ($.trim(im[ram_G[indexGame]])) {$( "#imaL" ).css("cursor","pointer");$( "#imaL" ).click(function() {imaUp("denaro3_resources/media/G" + im[ram_G[indexGame]])});}
$("#actText").css("minHeight",imaH[ram_G[indexGame]]);$(".buttonAu").remove();
if ($.trim(a1[ram_G[indexGame]])){$("#origin").append('<img class="buttonAu" src="denaro3_resources/media/sound.png"/>');
...
function paintScore(){
$("#scoreC p").text(String(totCentFixed/10).replace(".","").replace(",","")+" "+nameCent);$("#scoreU p").text(String(totEurosFixed/1000).replace(".","").replace(",","")+" "+nameEuro);
}
function randomSort(){
